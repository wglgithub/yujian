...
public class BaodanDao extends NativeBaseDao{
	...
	public Page<BaodanVo> findMyList(int page ,int size,  String userId){
		...
		if(total==0){
			...
			.addScalar("fahuoState", StandardBasicTypes.STRING)
	...
	public Page<BaodanVo> findMyList(String userId,String role,BaodanSearchParam p){
		...
		if(total==0){
			...
			.addScalar("paymentState1", StandardBasicTypes.STRING)
			.addScalar("paymentState2", StandardBasicTypes.STRING)
			.addScalar("wuliu", StandardBasicTypes.STRING)
			.addScalar("remark", StandardBasicTypes.STRING)
			.setResultTransformer(Transformers.aliasToBean(BaodanVo.class));
			content = query.getResultList();
	...
	Query findMyListQuery(String userId,String role,BaodanSearchParam p,String type){
		...
		if(type.equals("query")){
		    ...
		    .append("b.fahuoState as fahuoState, ")
		    .append("b.currentState as currentState, ")
		    .append("b.paymentState1 as paymentState1, ")
		    .append("b.paymentState2 as paymentState2, ")
			.append("case when u.role ='worker' and b.paymentState2='已回款' then b.payment2 WHEN (u.role ='agent' or u.role ='admin' ) and b.paymentState1='已回款' then b.paymentState1 else  b.fahuoState end  as state1 , ")
