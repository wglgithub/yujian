...
import com.topmobile.util.BaseHttpSSLSocketFactory.TrustAnyHostnameVerifier;
...
public class HttpClient {
	 ...
	 * @throws Exception
	 */
	public int send(String body, String encoding) throws Exception {
		try {
			HttpURLConnection httpURLConnection = createConnection(encoding);
			if(null == httpURLConnection){
				throw new Exception("创建联接失败");
			}
			this.requestServer(httpURLConnection, body,
					encoding);
			this.result = this.response(httpURLConnection, encoding);
